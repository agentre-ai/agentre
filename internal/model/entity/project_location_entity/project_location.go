...
// 仍住在 projects.path（避免双源同步）。账号内自然键是 (project, daemon_fingerprint)
// ——本表不存空 daemon_fingerprint 的行（决策 26）。
//
...
type ProjectLocation struct {
	...
	DaemonFingerprint string `gorm:"column:daemon_fingerprint;type:text;not null;default:''"`
	Path              string `gorm:"column:path;type:text;not null"`
	Status            int    `gorm:"column:status;type:int;not null;default:1"`
	Createtime        int64  `gorm:"column:createtime;type:bigint;not null;default:0"`
	Updatetime        int64  `gorm:"column:updatetime;type:bigint;not null;default:0"`
	// SyncMeta 账号级同步元数据（R1，366 行；决策 26 的自然键另见 daemon_fingerprint）。
...
// DaemonFingerprint 那一行（R2b：未配对）。取代原先的 IsLocal()：本表从不存放
