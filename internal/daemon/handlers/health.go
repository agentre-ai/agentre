...
//
// DBPath / DBSizeBytes 是这台 daemon 通知日志所在库文件的落盘位置与体量(规格
// 「安全、隐私、兼容性与可访问性 / 磁盘增长」):远端盒子上的 transcript 是档案,
// 体量得看得见,用户才判断得了何时该清理。没有库统计口时一并省略,而不是报 0 ——
// 「不知道」与「库是空的」在界面上必须是两回事。
...
func (h *HealthHandlers) Ping(_ context.Context) (HealthPingResult, error) {
	...
	if h.dbStat != nil {
		stat := h.dbStat.DBStat()
		res.DBPath, res.DBSizeBytes = stat.Path, stat.SizeBytes
	}
	return res, nil
}
