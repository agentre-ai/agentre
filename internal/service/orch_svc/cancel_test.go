...
func TestCancelTask_CascadesAndAborts(t *testing.T) {
	...
	tasks.EXPECT().Find(gomock.Any(), int64(2)).Return(
		&orch_entity.Dispatch{ID: 2, RunID: 100, SessionID: 800, Status: orch_entity.DispatchRunning, ParentDispatchID: 1}, nil)
	tasks.EXPECT().ListByRun(gomock.Any(), int64(100)).Return([]*orch_entity.Dispatch{
		{ID: 2, RunID: 100, SessionID: 800, Status: orch_entity.DispatchRunning, ParentDispatchID: 1, Kind: "dispatch"},
		{ID: 3, RunID: 100, SessionID: 801, Status: orch_entity.DispatchRunning, ParentDispatchID: 2, Kind: "dispatch"}, // 子孙
		{ID: 4, RunID: 100, SessionID: 802, Status: orch_entity.DispatchDone, ParentDispatchID: 2, Kind: "dispatch"},    // 已终态,跳过
	}, nil)
...
func TestCancelTask_RejectsForeignRun(t *testing.T) {
	...
	tasks.EXPECT().Find(gomock.Any(), int64(77)).Return(
		&orch_entity.Dispatch{ID: 77, RunID: 999}, nil)
