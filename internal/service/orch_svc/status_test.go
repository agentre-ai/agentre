...
func TestFormatRunStatus(t *testing.T) {
	tasks := []*orch_entity.Dispatch{
		{ID: 1, AgentID: 10, Kind: "dispatch", Status: orch_entity.DispatchAwaitingChildren, Brief: "根", CallSeq: 1, ParentDispatchID: 0},
		{ID: 2, AgentID: 11, Kind: "dispatch", Status: orch_entity.DispatchRunning, Brief: "前端", CallSeq: 1, ParentDispatchID: 1},
		{ID: 3, AgentID: 12, Kind: "dispatch", Status: orch_entity.DispatchDone, Brief: "后端", CallSeq: 1, ParentDispatchID: 1, Summary: "完工"},
	}
	...
	Convey("status JSON 覆盖任务树 + agent 名 + has_summary + blocked_on", t, func() {
		...
		So(out, ShouldContainSubstring, `"dispatch_id":2`)
...
func TestRunStatus_ScopesToCallerRun(t *testing.T) {
	...
	tasks.EXPECT().FindBySession(gomock.Any(), int64(500)).Return(
		&orch_entity.Dispatch{ID: 1, RunID: 100, SessionID: 500}, nil)
	tasks.EXPECT().ListByRun(gomock.Any(), int64(100)).Return([]*orch_entity.Dispatch{
		{ID: 1, RunID: 100, AgentID: 10, Kind: "dispatch", Status: orch_entity.DispatchRunning, Brief: "根"},
	}, nil)
