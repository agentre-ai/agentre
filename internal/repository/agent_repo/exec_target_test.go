...
// TestExecTargetReplace 写口:整表替换成给定顺序,sort_order 按下标落。
func TestExecTargetReplace(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO `agent_exec_targets`").
		WithArgs(int64(42), int64(7), 0, int64(42), int64(9), 1).
		WillReturnResult(sqlmock.NewResult(2, 2))
	mock.ExpectCommit()

	require.NoError(t, repo.Replace(ctx, 42, []int64{7, 9}))
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestCreateWritesSingleExecTarget(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO `agent_exec_targets`").
		WithArgs(int64(42), int64(7), 0).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()

	err := repo.Create(ctx, &agent_entity.Agent{Name: "Eva", DepartmentID: 1, AgentBackendID: 7, Status: consts.ACTIVE})
	require.NoError(t, err)
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestUpdateReplacesExecTarget(t *testing.T) {
	...
	mock.ExpectExec("DELETE FROM `agent_exec_targets` WHERE agent_id = \\?").
		...
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec("INSERT INTO `agent_exec_targets`").
		WithArgs(int64(42), int64(9), 0).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()

	err := repo.Update(ctx, &agent_entity.Agent{ID: 42, Name: "Eva", DepartmentID: 1, AgentBackendID: 9, Status: consts.ACTIVE})
	require.NoError(t, err)
	assert.NoError(t, mock.ExpectationsWereMet())
}
