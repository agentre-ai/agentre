...
func TestSessionFilePathIsDeterministicPerSession(t *testing.T) {
	p1 := agentruntime.PiAgentSessionFilePath("/data/pi-sessions", 7)
	p2 := agentruntime.PiAgentSessionFilePath("/data/pi-sessions", 7)

	assert.Equal(t, "/data/pi-sessions/agentre-7.jsonl", p1)
	assert.Equal(t, p1, p2, "same session id → same path → resume")
	assert.NotEqual(t, p1, agentruntime.PiAgentSessionFilePath("/data/pi-sessions", 8))
}

// 没有有效 session id（如连通性探测）时不强制 resume，返回空串。
func TestSessionFilePathEmptyWithoutSessionID(t *testing.T) {
	assert.Equal(t, "", agentruntime.PiAgentSessionFilePath("/data/pi-sessions", 0))
	assert.Equal(t, "", agentruntime.PiAgentSessionFilePath("", 7))
}
