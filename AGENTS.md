...
- [e2e/README.md](e2e/README.md) — the e2e / verification harness (root `e2e/` package): the **driven app** (`make verify-up` + `node e2e/drive.mjs <action>`, `fake` vs `real` flavor) that replaces authoring a spec for one-off checks, the small committed `e2e/tests/` core suite (`make e2e`), scratch specs for replay/timing (`make e2e-scratch TASK=…`, `AGENTRE_E2E_REUSE=1` reuses the running app), the isolation guards in `e2e/lib/target.mjs` and how they are enforced, the build-tag seam that keeps the fake out of production builds, the `node:sqlite` DB oracle, and how to extend the harness.
