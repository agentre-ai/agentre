...
**Hard invariant:** 本轮只扩展候选的匹配、排序与 `!` 历史补全；`/`、`$`、`@` 的触发边界和提交语义不得回退，历史候选不得自动执行 Shell 命令，`!` 命令与输出仍不得发送给 AI，历史持久化失败不得阻止命令执行，现有本地命令输出卡片仍只保留在当前应用运行期。
...
用户提交非空 `!` 命令时，系统记录提交分支实际交给本地命令执行回调的规范化完整文本。记录发生在命令提交时，不以启动成功、退出码或最终状态为条件；命令不存在、执行失败或返回非零退出码仍保留在历史中。仅浏览或选择候选不更新最近时间，真正再次提交该命令才将其移到最近位置。

## Persistent state and data lifecycle

新增一份版本化的本机历史数据。每个作用域由稳定执行设备身份与当前解析出的 cwd 共同确定；本地执行使用固定本机身份，远端执行使用稳定设备身份。cwd 为空时，同一设备的自由会话共享一个独立默认作用域。当前 cwd 或执行设备变化后，菜单立即读取新作用域，不混入旧作用域历史。
...
完整 Shell 命令可能含凭证、私有路径或其它敏感参数。本轮按用户确认保存原文，不做不可靠的自动脱敏；数据只保存在 Agentre 本机前端存储，不随聊天消息发送、不进入 AI 上下文、不新增后端请求或日志。作用域隔离和当前目录清理入口是本轮拥有的隐私边界。
...
- 不改变命令执行、自动发送、Mention XML、Agent 路由或项目引用语义。
...
该行为可以在纯函数、本地存储适配器和 TipTap 组件集成边界完整自动化；没有新增真实 Wails IPC、数据库、平台专属后端或像素级布局，因此本轮不新增 GUI E2E。持久化验证需要从空存储开始记录、重新构造读取方后确认恢复，再清理当前作用域并确认其它作用域未变；静态类型检查、前端全量测试与 lint 覆盖共享模块迁移、类型、i18n 和格式风险。
