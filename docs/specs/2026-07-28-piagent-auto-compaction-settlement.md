...
- 不为缺少 `agent_settled` 的非兼容 Pi RPC 实现增加回退；`agent_settled` 是本修复所依赖的外部 Pi RPC 协议前提。
...
- **兼容性**：以支持 `agent_settled` 的 Pi RPC 契约为基线；精确 Pi 版本与来源验证属于本地验收报告，不作为仓库事实；显式 compact 命令保留独立终止语义。
...
6. **将精确 Pi 版本和来源验证留给本地验收报告**；仓库规格只描述 `agent_settled` 外部协议前提，不把本机安装版本、更新时间或来源写成分支事实。
...
- **本地集成验证**：使用验收环境中支持 `agent_settled` 的真实 Pi RPC 或等价脚本，覆盖 `agent_end → compaction → agent_settled`；精确 Pi 版本与来源记录在本地验收报告中，并确认现有 `agentre-2223.jsonl` 可被 Pi 解析/resume；验证时不向原 session 写入测试数据。
...
When 验收环境中支持 `agent_settled` 的 Pi 以只读验证方式加载/检查该副本，或在隔离副本上 resume；<br>
