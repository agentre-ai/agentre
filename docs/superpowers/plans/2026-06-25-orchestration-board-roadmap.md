...
| S4 | **结构图视觉对齐** | 无 | `structure-graph.tsx` 校准：Leader 皇冠 + 自顶向下树 + subagent 扇出(`callSeq` 徽标) + 阻塞红框 + 死锁徽标 | 无 | 中 | 否 |
| S5 | **任务板汇总计数 5/12** | 无（前端按 task 状态聚合） | task-board 头部 done/total 徽标 + agent/subagent 分组 | 无 | 小 | 否 |
| S6 | **进入会话（钻入·只读）** | 无（每 task 有 `SessionID`，读该 session 消息） | 节点/任务行/头像点击 → 右栏「任务板 ⇄ 会话」切换；transcript + 「对它说」 | 无 | 中 | 否 |
