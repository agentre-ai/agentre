...
- 2026-06-03 更新：寻址机制由"文本解析 `<mention>`"升级为 **MCP `group_send` tool**（结构化收件人），并新增**能力门控** `CapGroupChat`（MVP 仅 claudecode）。`<mention>` 标记保留，仅用于**前端展示高亮 + 点击跳转**。
...
| 发消息机制 | **agent 调 `group_send(body, mentions[])` MCP tool 发言**（一次调用 = 一条群消息，一个 turn 内可多次、可对不同人发不同内容）。成员的私有叙述/思考**不进群**，只有 tool 调用进群。门控:**只有声明 `CapGroupChat` 能力的 backend(MVP 仅 claudecode)可入群** |
...
—— 对现有代码的改动（seam）：
internal/pkg/agentruntime/capability/         + CapGroupChat 常量
internal/pkg/agentruntime/runner.go           RunRequest + MCPServers 字段
internal/pkg/agentruntime/runtimes/claudecode/  Capabilities() 声明 CapGroupChat + 映射 MCPServers
...
1. **新能力 `CapGroupChat`**（`capability.go` 加常量 `CapGroupChat Capability = "group_chat"`）。语义 = **该 runtime 接受 `RunRequest.MCPServers` 并带 MCP tool 启动**。`claudecode.Capabilities().Set[CapGroupChat]=true`；codex/builtin/piagent 不声明。matrix 测试同步（agent-backend.md §0.5 三处同步：runtime.go / capability.go / runtime_test.go）。
...
> 门控落点：`group_svc.AddGroupMember`/`EnsureGroupMemberSession` 加成员前，查该 agent 绑定 backend 的 `Capabilities().Has(CapGroupChat)`，false → 报 `code.GroupBackendUnsupported`。前端邀请/选协调者按 cap 过滤 agent（复用现有 `GetBackendCapabilities`，hook 不改，只改消费端）。
...
- **招募**：协调者 `group_send` 的 `mentions` 里出现**在部门名单、尚未进群**的 agent 名 → `group_svc` 自动新增成员（先查 `CapGroupChat`，不支持则拒绝并 flag）、`EnsureGroupMemberSession` 起 backing session、post 一条**系统消息**"X 加入"（`sender_kind=system`，投递给全体在群成员做 in-band 名单更新），并把触发的这条消息投递给新成员。成员数上限 ~8。**MVP 仅协调者能触发自动招募**（非协调者 mention 名单外/未进群 agent → 忽略并 flag）；用户也可在 UI 手动加任意**支持 `CapGroupChat` 的** agent。
...
- `AddGroupMember(req)`（仅可加支持 `CapGroupChat` 的 agent）/ `RemoveGroupMember(req)`
...
- **右 · 群信息**：分 **成员 / 设置** 两个 tab（成员/设置小 tab 用 Button+state；仓库无 shadcn `Tabs`）。成员 tab = 协调者 + 成员（实时状态：思考中/待批准/空闲）+ "邀请成员"（**仅列支持 `CapGroupChat` 的 agent**）；设置 tab = 工作目录 / 归档群。**每个成员行可点（尾部 `›`）→ 跳转到它的完整 backing session 会话**（复用单聊视图，看全程工具调用/思考/改动；该会话以新视图 tab 打开），目标视图顶部「← 返回群聊」。
...
- **能力 / runtime 单测**：`capability` matrix 测试同步 `CapGroupChat`；claudecode `Capabilities()` 声明 + RunRequest.MCPServers → mcp-config 映射的单测（不 spawn 真子进程，断言 args/options，仿 `ccBuildClientOpts` 现有测试）；`pkg/claudecode` args 加 `--mcp-config` 的单测。
...
- **Service 单测**：mockgen 生成 `mock_group_repo`，并 mock `chat_svc` 的 `EnsureGroupMemberSession` / `Send` / `ObserveTurn` / `Stop` seam（注入窄接口 `ChatGateway`）。BDD/goconvey 覆盖：
  ...
  - 招募流：协调者 `group_send` mention 名单内未进群 agent（且支持 CapGroupChat）→ 新成员 backing session 建立 + 系统消息;非协调者触发 → 忽略并 flag;不支持 CapGroupChat → 拒绝并 flag。
  - 门控：加不支持 `CapGroupChat` 的 agent → `code.GroupBackendUnsupported`。
...
3. **能力门控 `CapGroupChat`**：只有声明该能力的 backend 可入群；语义 = runtime 接受 `RunRequest.MCPServers`。group_svc + 前端按 cap 过滤。（待确认能力命名 `CapGroupChat` vs `CapMCPTools`，§14.8）
...
5. **招募 = 协调者 `group_send` mention 名单内未进群且支持 CapGroupChat 的 agent**（仅协调者可触发）；可招募名单 = 部门成员；成员上限 ~8。
...
9. **domain 包名 `group_*`**（vs `crew_*`/`squad_*`）；**能力名 `CapGroupChat`**（vs 机制名 `CapMCPTools`）—— 待确认。
...
- 能力门控经 `CapGroupChat`，**不在 chat_svc/group_svc 写 `if type==claudecode`**（OCP）。
