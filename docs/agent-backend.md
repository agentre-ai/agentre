...
- **piagent** — wraps the local `pi` CLI (Pi coding agent RPC mode); it is not bound to an Agentre `LLMProvider`, but reads Pi's own `~/.pi/agent` config and auth. Supports steer / abort / compact / image input / context-window reporting / fork-session; Pi owns the Session storage location, while Agentre persists the native Session ID returned by RPC plus each turn's native user-entry anchor, and resumes with `--session <id>`. It does not support tool approval, reverse Q&A, or permission mode meta.
